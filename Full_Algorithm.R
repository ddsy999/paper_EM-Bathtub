...
annealingSchedule = c(seq(0.6,0.9999,length.out=maxIterAnnealing),1)[2:(maxIterAnnealing+1)]
bpBaseSchedule = seq(initialbpBase,1e+3,length.out=maxIterAnnealing)^2
...
for( ITerAnneal in 1:maxIterAnnealing){
  ...
  for( iter in 1:maxEMIter){ 
    ...
    # new_beta1 = barrier_beta1(candi_before_vec[1],latentZ_mat,bp=bpBaseSchedule[ITerAnneal] )
    # new_beta3 = barrier_beta3(candi_before_vec[3],latentZ_mat,bp=bpBaseSchedule[ITerAnneal] )
    # new_beta1 = barrier_beta1(candi_before_vec[1],latentZ_mat,bp=1e+6 )
    # new_beta3 = barrier_beta3(candi_before_vec[3],latentZ_mat,bp=1e+6 )
...
# appAnnealLimit = theta_df_full$tempering[min(which(theta_df_full$Qlike > -573))]
...
optimalData = theta_df_full %>% tail(1)
