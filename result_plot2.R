...
optimalEnd = which.min(abs(roadResult$diffBeta1))

A1=roadResult %>% ggplot(aes(x=Iter , y=Q1+Q2))+geom_point()+geom_line()+labs(title="Expected log-likelihood",y="",x="")+theme(axis.text = element_text(size=8), plot.title = element_text(size = 12))+geom_vline(xintercept = optimalEnd,color="red",lty=2)
...
hzPlot = hzData %>% ggplot(aes(x=time , y=hz1))+geom_line(color="red")+
  ...
  # geom_line(aes(x=time ,y=hz3),color="blue")+
