...
n_burnIn = 0.6
n_wearOut = 0.4
n_burn <- floor(n_total*n_burnIn)
n_wear <- n_total - n_burn


shape_1 = 0.5478
# lambda_1 = 0.3574
lambda_1 = 0.3574
scale_1 = lambda_1^(-1/shape_1)

# shape_3 = 78
# lambda_3 = 2.18e-151
shape_3 = 7
lambda_3 = 5e-12
scale_3 = lambda_3^(-1/shape_3)

time <- c(rweibull(n_burn, shape = shape_1, scale = scale_1),
          rweibull(n_wear, shape = shape_3, scale = scale_3))
event <- rep(1, length(time))

df <- data.frame(time = time, event = event)
df$distn = c(rep("burn",n_burn),rep("wear",n_wear))
wearMax =df %>% filter(distn=="wear") %>% pull(time) %>% max
df = df %>% mutate(time=ifelse(time>wearMax,wearMax,time)) %>% mutate(event = ifelse(time>=wearMax,0,1))
df = df %>% arrange(time)


df = left_join(df ,compute_empirical_hazard_surv(df) , by="time" )


# hazard point plot
ggplot(df, aes(x = time, y = hazard_rate ,color=distn )) +
  geom_point() +
...
bpBaseSchedule =exp(seq(log(1e-1), log(1e+12), length.out = maxIterAnnealing))
...
# initial_beta = c(1,35)
initial_beta = c(0.5,10)
initial_pi_set = c(1,1)
initial_pi = initial_pi_set / sum(initial_pi_set)

initial_lambda = initial_lambda_func(time_vec,event_vec,beta_vec=c(initial_beta[1],1,initial_beta[2]),ratio1=0.5,ratio3=0.7)
initial_lambda = c(initial_lambda[1],initial_lambda[3])
...
for( ITerAnneal in 1:maxIterAnnealing){
  ...
  for( iter in 1:maxEMIter){ 
    ...
    # print(new_beta3)
    #### Update Parameter ####
    new_beta = c(new_beta1,new_beta3)
...
optData = theta_df_full[which.min(theta_df_full$diffBeta3 %>% abs+theta_df_full$diffBeta1 %>% abs),]
optData

trueDF = data.frame(beta1=shape_1,lambda1=lambda_1 ,beta3=shape_3,lambda3=lambda_3 
                    ,pi1=n_burn/(n_burn+n_wear) , pi3= n_wear/(n_burn+n_wear))
trueDF

df$hz2 = hazardrate(df$time,optData$beta1,optData$lambda1)
